                ...
                <table className="w-full text-xs text-zinc-300 border-collapse">
                  <thead>
                    <tr className="border-b border-zinc-800 text-zinc-500 text-[9px] uppercase tracking-wider">
                      ...
                      <th className="py-2.5 px-2 font-bold w-20 text-center">GODINA</th>
                    ...
                    {activeEpisodes.map((ep) => (
                      <tr key={`bulk-row-${ep.id}`} className="hover:bg-zinc-850/25 transition">
                        ...
                        <td className="py-2 px-2">
                          <input
                            type="text"
                            value={ep.releaseYear || ep.airDate || ''}
                            onChange={(e) => {
                              handleEpisodeChange(ep.id, 'releaseYear', e.target.value);
                              handleEpisodeChange(ep.id, 'airDate', e.target.value);
                            }}
                            className="w-16 bg-zinc-950 border border-zinc-800 rounded px-2 py-1.5 text-xs text-yellow-400 font-mono font-bold text-center placeholder-zinc-700 focus:outline-none focus:border-yellow-450"
                            placeholder="2024"
                          />
                        </td>
              ...
              <div className="block md:hidden space-y-3">
                {activeEpisodes.map((ep) => (
                  <div key={`bulk-card-${ep.id}`} className="bg-zinc-950/40 p-4 border border-zinc-850 rounded-xl space-y-3 text-left">
                    ...
                    <div className="space-y-1.5">
                      <label className="block text-[9px] uppercase font-bold text-zinc-500">Godina izlaska epizode</label>
                      <input
                        type="text"
                        value={ep.releaseYear || ep.airDate || ''}
                        onChange={(e) => {
                        ...
                        className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-xs text-yellow-400 font-mono font-bold"
